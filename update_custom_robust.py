...
# Backup automatico
backup = FILE.with_suffix(".html.custom-backup")
shutil.copy2(FILE, backup)

s = FILE.read_text(encoding="utf-8")

def replace_between(start_pattern, end_pattern, replacement, label):
    global s
    m1 = re.search(start_pattern, s, flags=re.I)
    if not m1:
        raise SystemExit(f"ERRORE: inizio '{label}' non trovato. Backup: {backup}")
    m2 = re.search(end_pattern, s[m1.end():], flags=re.I)
    if not m2:
        raise SystemExit(f"ERRORE: fine '{label}' non trovato. Backup: {backup}")
    start = m1.start()
    end = m1.end() + m2.start()
    s = s[:start] + replacement + s[end:]

# HERO: dalla prima page-hero alla sezione successiva
...
"""
replace_between(r'<section[^>]*class=["\'][^"\']*page-hero[^"\']*["\'][^>]*>',
                r'</section>',
                hero, "hero")

# Price: se esiste una price-band la sostituiamo; altrimenti la inseriamo dopo hero.
...
"""
if re.search(r'<section[^>]*class=["\'][^"\']*price-band', s, flags=re.I):
    replace_between(r'<section[^>]*class=["\'][^"\']*price-band[^"\']*["\'][^>]*>',
                    r'</section>', price, "price-band")
else:
    pos = s.find("</section>", s.find('class="page-hero"'))
    if pos != -1:
        pos += len("</section>")
        s = s[:pos] + "\n" + price + s[pos:]

# First main paper section after price
...
"""
# Inserisce details subito dopo price, evitando dipendenze dalla vecchia struttura.
m = re.search(r'<section[^>]*class=["\'][^"\']*price-band[^"\']*["\'][^>]*>.*?</section>', s, flags=re.I|re.S)
if m:
    insert_at = m.end()
    s = s[:insert_at] + "\n" + details + s[insert_at:]
else:
    s = s.replace("</main>", details + "\n</main>", 1)
...
"""
# Sostituisce il primo signal esistente; se non c'è, aggiunge dopo details.
if re.search(r'<section[^>]*class=["\'][^"\']*signal[^"\']*["\']', s, flags=re.I):
    replace_between(r'<section[^>]*class=["\'][^"\']*signal[^"\']*["\'][^>]*>',
                    r'</section>', signal, "signal")
else:
    s = s.replace(details, details + signal, 1)

# Rimuove eventuali vecchie sezioni di servizio/feature prima del FAQ e inserisce le nuove sezioni.
...
# Inserisce queste sezioni appena prima della FAQ, mantenendo FAQ/footer originali.
faq_match = re.search(r'<section[^>]*id=["\']faq["\'][^>]*>', s, flags=re.I)
if faq_match:
    pos = faq_match.start()
    s = s[:pos] + capabilities + scope + process + s[pos:]
else:
    # fallback: prima del footer
    pos = s.lower().find("<footer")
    if pos == -1:
        pos = s.lower().find("</main>")
    if pos == -1:
        raise SystemExit(f"ERRORE: impossibile trovare punto di inserimento. Backup: {backup}")
    s = s[:pos] + capabilities + scope + process + s[pos:]
...
"""
if re.search(r'<section[^>]*id=["\']faq["\'][^>]*>.*?</section>', s, flags=re.I|re.S):
    s = re.sub(r'<section[^>]*id=["\']faq["\'][^>]*>.*?</section>',
               faq, s, count=1, flags=re.I|re.S)
else:
    pos = s.lower().find("<footer")
    s = s[:pos] + faq + s[pos:]

# CTA: trova il blocco con "07 · START" e sostituisce fino alla chiusura della relativa section.
cta = r"""<section class="cta" id="start">
<div>
...
"""
cta_match = re.search(r'<section[^>]*class=["\'][^"\']*cta[^"\']*["\'][^>]*id=["\']start["\'][^>]*>.*?</section>',
                      s, flags=re.I|re.S)
if not cta_match:
    cta_match = re.search(r'<section[^>]*id=["\']start["\'][^>]*class=["\'][^"\']*cta[^"\']*["\'][^>]*>.*?</section>',
                          s, flags=re.I|re.S)
if cta_match:
    s = s[:cta_match.start()] + cta + s[cta_match.end():]
else:
    pos = s.lower().find("<footer")
    s = s[:pos] + cta + s[pos:]
...
print()
print("NON fare ancora git push. Prima controlliamo la pagina.")
