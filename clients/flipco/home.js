...
const homePieces=document.querySelector('#homePiecesGrid');
 ...
 const heroSlides=[
  {image:'assets/editorial/home/flipco-campaign.jpg',brand:'FLIP&CO',name:'THE EDIT',price:'CAGLIARI',link:'shop.html'},
  {image:'assets/editorial/home/flipco-donna.jpg',brand:'CAGLIARI / ITALIA',name:'NEW SEASON',price:'ONLINE EDIT',link:'collections.html'},
  {image:'assets/editorial/home/flipco-kids.jpg',brand:'FLIP&CO / KIDS',name:'NEXT GENERATION',price:'CAGLIARI',link:'shop.html?category=Kids'}
 ];
 let hi=0;
...
/* V42 — experience layer: reveal, magnetic CTAs, pointer glow */
(function(){
  "use strict";
  const init=()=>{
    document.documentElement.classList.add("flip-v42");

    const revealSelectors=[
      ".f29-brand-wall",".f29-finder",".f29-story",".flip-lookbook",
      ".f51-pieces",".f29-close",".f29-edit"
    ];
    revealSelectors.forEach(sel=>{
      document.querySelectorAll(sel).forEach(el=>el.classList.add("v42-reveal"));
    });
      ...
      const io=new IntersectionObserver(entries=>{
        entries.forEach(entry=>{
          if(entry.isIntersecting){
            entry.target.classList.add("v42-visible");
            io.unobserve(entry.target);
          }
        });
      },{threshold:.12,rootMargin:"0px 0px -7% 0px"});
      document.querySelectorAll(".v42-reveal").forEach(el=>io.observe(el));
    }else{
      document.querySelectorAll(".v42-reveal").forEach(el=>el.classList.add("v42-visible"));
    }

    // Magnetic movement is intentionally subtle and disabled for coarse pointers.
    if(window.matchMedia("(pointer:fine)").matches){
      document.querySelectorAll(".f29-btn,.f29-textlink,.f29-underlink,.flip-look-product-link").forEach(btn=>{
    ...
    // Soft pointer glow for the Shop the Look stage.
    const look=document.querySelector(".flip-lookbook");
    if(look && window.matchMedia("(pointer:fine)").matches){
      look.addEventListener("pointermove",e=>{
        const r=look.getBoundingClientRect();
        look.style.setProperty("--mx",`${e.clientX-r.left}px`);
        look.style.setProperty("--my",`${e.clientY-r.top}px`);
      });
    }
  };
